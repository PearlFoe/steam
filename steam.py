...
def get_item_data(app_id, item_name):
	url = f'https://steamcommunity.com/market/priceoverview/?appid={app_id}&market_hash_name={item_name}'
	data = requests.get(url).json()

	return data

def get_item_price_histogram(app_id, item_name):
	...
	time.sleep(3)
	url = f'https://steamcommunity.com/market/itemordershistogram?country=RU&language=russian&currency=1&item_nameid={item_nameid}&two_factor=0&norender=1'
	data = requests.get(url).json()
